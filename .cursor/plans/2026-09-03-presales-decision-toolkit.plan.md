...
overview: "把项目既有四象选型框架做成可交互单文件 HTML 售前物料 + 配套话术手册，用于第一次客户会议的需求定位。"
todos: []
isProject: false
---
...
**Architecture:** 单文件 HTML（内联 CSS/JS，零依赖零构建）+ 一份 Markdown 话术手册。产品口径以 `docs/enterprise-landing.md` 的四象定位、决策问题、场景样例、组合模式与选型边界为准；所有对外文字均应独立编写。
...
- 项目既有四象选型框架采用逐层分诊：逻辑推理→Semantic / 多跳关联→KG Schema / 操作闭环→执行层 / Agent 约束→治理层；场景表与选型边界由项目结合公开标准和自身产品定位独立整理。
...
- **FR-1 决策树四问交互**：单页依次呈现 Q1~Q4（Q1 需要逻辑推理吗 / Q2 需要多跳关联查询吗 / Q3 需要操作执行闭环吗 / Q4 需要约束 Agent 行为吗），每问只有 是/否 两个按钮；任一问答「是」即终止并输出对应象限推荐（Q1→Semantic Ontology、Q2→KG Schema、Q3→执行层、Q4→治理层）；四问全「否」输出治理层兜底建议（项目现有产品规则）。
- **FR-2 象限结果页**：每个象限展示——核心命题（一句话）、典型技术栈、我们的角色映射（`docs/enterprise-landing.md` §一点五表格内容：卖什么/不卖什么/走生态接口）、推荐 POC 类型（检索型/操作闭环型/治理型/转生态合作）。
- **FR-3 16 场景匹配表**：呈现项目独立整理的 16 行场景表（业务场景/核心需求/推荐本体论/选型理由），支持按象限 tab 筛选 + 顶部关键词搜索框（前端过滤，子串匹配即可）；每条建议须能由公开资料或项目验证记录说明依据。
- **FR-4 组合模式说明区**：展示知识层+查询层 / 数据层+执行层 / 业务层+治理层三种项目组合模式的文字流程图，并说明先验证单层价值、再按实际需求叠加的演进原则。
- **FR-5 四大红线区**：以项目自身语言说明四类选型风险，每条配「售前话术：遇到该诉求如何回应」。
...
| 产品口径 | `docs/enterprise-landing.md` 与公开标准链接 | 独立编写并保留可核验依据 |
...
16 场景表数据结构（内嵌 JSON；内容由项目独立整理，不依赖受限资料的行序或措辞）：
...
**Out of scope（no-scope-creep）：** 不做后端、不做用户行为埋点、不做多语言、不扩写 `enterprise-landing.md` 既有章节（只加一行链接）、不做品牌视觉规范；不纳入受限资料原文、全文转写稿或可还原其表达的内容。
