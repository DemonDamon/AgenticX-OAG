...
> 版本：v0.2（2026-09）——v0.2 基于项目研究形成四象定位、选型问题、场景匹配与组合模式；对外口径以公开标准、厂商公开资料和项目验证结果为依据。
...
**公开比较对象**：Semantica。按仓库锁定源码，Semantica 覆盖知识图谱、本体、检索、规则推理、决策记录与可选溯源等能力；AgenticX-OAG 计划在此基础上形成独立的运行时治理能力。
...
项目研究把当前技术语境下的「本体」区分为四类，各有不同的核心命题、技术栈与工程边界。**这套四象框架是客户对话的分诊语言——客户说「我们要建本体」时，必须先确认实际需要解决的问题。** 我们的产品定位据此校准：
...
| **Palantir Ontology**（执行层）          | 业务怎么运转——Object+Action 驱动操作闭环 | ✅ 公开能力参照。项目计划建设 Action Gateway + Function + Writeback，并以实现和测试验证覆盖范围 |
| **Agent Ontology**（治理层）             | Agent 该怎么做——约束规约智能体行为边界      | ✅ 计划差异化。为 LLM 提案增加执行前的运行时门禁；按仓库锁定源码，Semantica 尚未证明具备企业级授权、审批与外部写回网关的完整闭环 |

**一句话定位：AgenticX-OAG = KG Schema（查询层）+ Palantir 式执行层 + Agent Ontology 治理层的三层组合体，Semantic Ontology 走生态接口。**

这个定位由项目的产品边界与以下组合关系共同推导：
...
**售前对话规则**：第一次客户会议不谈产品，先用项目的四个分诊问题定位客户需求（见下节）。客户要解决的核心问题决定我们卖什么、不卖什么。
...
配套项目独立整理的 16 场景匹配表做参照（反欺诈→KG、运行控制→执行层、维修工卡 Agent→治理层、智能客服→组合）。**这张表应做成售前物料，帮助客户对照需求，但结论仍需结合现场约束验证。**
...
**演进原则（项目工程建议）**：先做深经 POC 验证的象限，再按客户实际痛点叠加第二个象限。避免一开始建设三层全栈，以控制架构复杂度和交付风险。
...
**治理双维度（v0.2 新增，项目既有设计）**：治理 = 权限维度 × 语义约束维度，两者正交、缺一不可：
...
项目据此把确定性 Action 的执行治理与概率性 LLM 的提案治理分层。
按当前公开资料核验，Semantica 主要覆盖知识图谱、本体与检索能力；
Agent 提案的运行时门禁是 AgenticX-OAG 计划形成的差异化能力。

### 3.2 与公开产品及方案的能力比较

> 证据边界：Semantica 一列依据仓库锁定公开源码的静态审阅，不代表运行验证；Palantir 一列依据官方公开文档。AgenticX-OAG 一列为路线图目标，不能作为已交付能力声明。
...
| 决策治理  | ❌ 不适用       | ⚠️ 依具体框架        | ⚠️ 有决策记录、政策与批准链接口；企业授权/审批闭环未由锁定源码证明 | ✅ Actions + Functions | 🧭 计划：Action Gateway + 审批 |
| 审计追溯  | ⚠️ 引用片段    | ⚠️ 依具体框架        | ⚠️ 可选 PROV-O 与决策记录 | ✅ 官方提供治理与审计能力 | 🧭 计划：PROV-O + 回放         |
| 低代码搭建 | ❌ 不适用       | ⚠️ 依具体框架        | ⚠️ Explorer      | ✅ Workshop            | 🧭 计划：Ontology Workshop   |
| 商业形态    | 依实现而定          | 依实现而定           | 开源项目             | 企业商业平台              | 计划：开源 + 商业服务             |
| 部署方式  | SaaS/私有    | 本地/私有       | 自托管              | 私有云/本地                | 自托管 + 私有部署            |

**目标卖点：以开源、可自托管的方式组合本体、执行治理与 Agent 提案门禁；具体能力与成本须以实现和评测结果为准。**
...
**预置行业模板**（基于项目目标市场与公开案例筛选，每行业标注主打象限）：

- 金融：反洗钱、风控、合规、客户画像 ——主打 KG Schema + 执行层（示例：反欺诈多跳团伙发现）

- 制造：设备管理、质量管理、供应链 ——主打执行层 + Agent Ontology（示例：供应链调度操作闭环、设备维修 Agent 行为约束）

- 民航/交通：运行控制、维修工卡、适航管理 ——可覆盖多个象限；适航审查可作为 Semantic Ontology 生态接口示范

- 医疗：诊疗辅助、药物审查 ——主打 Agent Ontology（示例：用药禁忌拦截；仅作技术演示，不替代临床判断）
...
**这是项目计划形成的落地加速器。** 是否优于 Semantica 或 Palantir 的行业交付能力，需在模板实现后按公开能力和真实项目指标比较。
...
**核心逻辑**：开源做生态和标杆，商业版和服务形成收入。Palantir 采用企业商业平台与专业服务模式；本项目计划以开源降低采用门槛，再通过服务和行业模板创造价值。
...
以下内容由项目结合自身架构边界独立整理为销售与交付红线：

1. **知识图谱只承担适合它的职责**。实例关联与路径分析交给图层；形式推理、事务型 Action 和 Agent 策略分别由可验证的专用组件负责。
2. **概念模型与实例数据明确分层**。TBox 和业务实例采用不同生命周期与存储策略，并以受版本控制的映射衔接，避免推理边界与查询负载相互牵制。
3. **世界假设写入契约**。开放世界与封闭世界对缺失事实、否定查询和数据完备性的解释不同；接口、查询和测试必须声明所用假设及适用范围。
4. **多层能力按证据扩展**。POC 先为一个核心方向建立指标基线，再依据真实瓶颈增加下一层；每次扩展都要有验收条件和回退路径。
...
| 编号  | 名称                        | Plan 文件（实施中/已完成移至 `.cursor/plans/` 根，未开工在 `pending/`）  | 依赖                                                      | 核心交付物                                                                          | Roadmap    | 落地阶段   | 状态          |
| --- | ------------------------- | ----------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------ | ---------- | ------ | ----------- |
| P01 | 售前决策工具包                   | `2026-09-03-presales-decision-toolkit.plan.md`  | —（独立）                                                   | 四象决策树交互物料 + 话术手册                                                               | §七 调整 5    | 售前/全程  | done        |
| P02 | 引擎地基                      | `2026-09-03-engine-foundation.plan.md`          | —（**DAG 根**）                                            | Python 包骨架 + 本体元模型 Proto + 九接口 Protocol + CI/dev 环境                            | Phase 0    | 基建     | done        |
| P03 | 本体核心数据模型                  | `2026-09-03-ontology-core-model.plan.md`        | P02                                                     | Pydantic 镜像模型 + YAML/OWL/SHACL + AML 本体模板 + 场景转换器                              | Phase 1    | 阶段一    | done        |
| P04 | KG 构建器 + 图存储              | `2026-09-03-kg-builder-graph-store.plan.md`     | P02、P03                                                 | AGE 存储实现 + 本体引导 LLM 抽取 + 实体消歧 + 摄入管道                                           | Phase 1    | 阶段一    | done        |
| P05 | OAG 检索 + Context Pack     | `2026-09-03-oag-retrieval-context-pack.plan.md` | P02（接口全 mock，可与 P03/P04 并行）                             | 混合检索 + Pack 流水线 + 受引用约束的 OAG 生成器                                               | Phase 2    | 阶段一    | done        |
| P06 | POC 评测工具包                 | `2026-09-03-poc-evaluation-toolkit.plan.md`     | P04、P05（QA 数据集可先行：基于 prototype 场景 JSON 出题）              | 指标 + 三场景 QA 数据集 + OAG vs RAG A/B Runner + 一键报告                                 | Phase 2    | 阶段一    | done        |
...
**本体工程演进工作集（分支 `feat/oag-ontology-evolution`）**：P02-P06 已形成基础能力，当前以 P07-P11 为承载计划，补充 Action 契约与补偿、Harness 四层边界、四权与决策回放、AML 场景包、Object View/Action/权限继承等要求。任务分解和公开依据见 [`ontology-evolution-backlog.md`](ontology-evolution-backlog.md)。该工作集只增强既有 plan，不改变依赖 DAG，P07-P11 仍保持 `pending`，开始实施时再依 §8.5 流转状态。
...
- 以开源、可自托管的组件逐步形成本体 + 治理 + 低代码能力

- 治理双维度：四权矩阵（权限）× Agent Harness 四层（语义约束）；其中 Agent 提案运行时门禁是本项目计划形成的差异化能力
